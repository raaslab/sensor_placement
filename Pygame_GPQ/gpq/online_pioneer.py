...
def choose_action(curr_state):
	...
	return random.choice(list(np.where(q_pred == np.max(q_pred))[0])), var
	...
	while num_of_steps <= 200:
		if num_of_steps != 1:
			...
			if randomNumber >= epsilon:
				action, var = actions[choose_action(curr_state)]
			else:
				action = random.randint(-9, 9)		
...
curr_state = np.array([round(2*k) for k in next_state_laser_data])
...
for epoch in range(1, 31):
	...
	while num_of_steps <= 10:
		randomNumber = random.random()
		if randomNumber >= epsilon:
			action = actions[choose_action(np.array(curr_state))] 
		else:
			action = random.randint(-9, 9) 		
		...
		next_state = np.array([round(2*k) for k in next_state_laser_data])
		print next_state
